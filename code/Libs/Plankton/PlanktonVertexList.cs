...
public partial class PlanktonMesh<TFaceTraits, THalfedgeTraits, TVertexTraits>
{
	public class PlanktonVertexList : IEnumerable<PlanktonVertex>
	{
		...
		internal PlanktonVertexList( PlanktonMesh<TFaceTraits, THalfedgeTraits, TVertexTraits> owner )
		{
		...
		{
			return this.Add( new PlanktonVertex( position.x, position.y, position.z ) );
		}
		#endregion

		public int[] AddVertices( IEnumerable<Vector3> vertices )
		{
		...
		public bool OffsetVertex( int vertexIndex, float x, float y, float z )
		{
			if ( vertexIndex >= 0 && vertexIndex < _list.Count )
			{
				var v = _list[vertexIndex];
		...
		{
			var vertex = this[index].Position;
			...
			for ( int i = 0; i < n - 1; i++ )
			{
				normal += Vector3.Cross(
					this[ring[i]].Position - vertex,
					this[ring[i + 1]].Position - vertex );
			}

			if ( this.IsBoundary( index ) == false )
			{
				normal += Vector3.Cross(
					this[ring[n - 1]].Position - vertex,
					this[ring[0]].Position - vertex);
			}
		...
		{
			return Enumerable.Range( 0, this.Count ).Select( i => this[i].Position ).ToArray();
		}
		...
		public int SplitVertex( int first, int second )
		{
			...
			int v_new = this.Add( this[v_old].Position ); // copy vertex by converting to XYZ and back
