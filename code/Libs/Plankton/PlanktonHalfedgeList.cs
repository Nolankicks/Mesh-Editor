...
public partial class PlanktonMesh<TFaceTraits, THalfedgeTraits, TVertexTraits>
{
	public class PlanktonHalfEdgeList : IEnumerable<PlanktonHalfedge>
	{
		...
		internal PlanktonHalfEdgeList( PlanktonMesh<TFaceTraits, THalfedgeTraits, TVertexTraits> owner )
		{
		...
		/// <returns>The length of the halfedge, or -1 if unused</returns>
		{
			...
			{
				var Start = _mesh.Vertices[this[index].StartVertex].Position;
				var End = _mesh.Vertices[this.EndVertex( index )].Position;
		...
		public int SplitEdge( int index )
		{
			...
			int new_vertex_index = _mesh.Vertices.Add( _mesh.Vertices[end_vertex].Position ); // use XYZ to copy            
