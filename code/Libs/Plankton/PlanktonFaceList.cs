...
public partial class PlanktonMesh<TFaceTraits, THalfedgeTraits, TVertexTraits>
{
	...
	{
		private readonly PlanktonMesh<TFaceTraits, THalfedgeTraits, TVertexTraits> _mesh;
		...
		internal PlanktonFaceList( PlanktonMesh<TFaceTraits, THalfedgeTraits, TVertexTraits> owner )
		{
		...
		public void ReplaceFace( int face, IEnumerable<int> indices, TFaceTraits traits = default( TFaceTraits ) )
		{
			RemoveFace( face );
			var f = AddFace( indices, traits, face );
			if ( f == null || f.IsUnused ) return;
			this._list[face] = f;
		}
		...
		public int AddFace( IEnumerable<int> indices, TFaceTraits traits = default( TFaceTraits ) )
		{
			var face = AddFace( indices, traits, this.Count );
			if ( face == null || face.IsUnused ) return -1;
		...
		/// <returns>The index of the newly added face (-1 in the case that the face could not be added).</returns>
		/// <remarks>The mesh must remain 2-manifold and orientable at all times.</remarks>
		private PlanktonFace AddFace( IEnumerable<int> indices, TFaceTraits traits, int faceIndex )
		{
			...
			for ( int i = 0, ii = 1; i < n; i++, ii++, ii %= n )
			{
				...
				{
					int v1 = array[i], v2 = array[ii];
					loop[i] = hs.AddPair( v1, v2, faceIndex );
				}
				else
				{
					// Link existing halfedge to new face
					hs[loop[i]].AdjacentFace = faceIndex;
				}
			}
					...
					if ( hs[loop[i]].NextHalfedge != loop[ii] || hs[loop[ii]].PrevHalfedge != loop[i] )
					{
						...
						catch ( InvalidOperationException )
						{
							throw new InvalidOperationException( string.Format(
								"Failed to relink halfedges around vertex #{0} during creation of face #{1}", v2, faceIndex ) );
						}
					}
				}
			}
			...
			PlanktonFace f = new PlanktonFace() { FirstHalfedge = loop[0], Traits = traits };
		...
		/// <remarks>The mesh must remain 2-manifold and orientable at all times.</remarks>
		public int AddFace( int a, int b, int c, TFaceTraits traits = default( TFaceTraits ) )
		{
			return this.AddFace( new int[] { a, b, c }, traits );
		}
		...
		/// <remarks>The mesh must remain 2-manifold and orientable at all times.</remarks>
		public int AddFace( int a, int b, int c, int d, TFaceTraits traits = default( TFaceTraits ) )
		{
			return this.AddFace( new int[] { a, b, c, d }, traits );
		}
		...
		{
			var faceVertices = GetFaceVertices( f );
